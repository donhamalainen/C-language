...
PNG brighten(PNG original, int amount)
{
    ...
    for (int yi = 0; yi < original.height(); yi++)
    {
        ...
        {
            int r = original(xi, yi)->red + amount;
            int b = original(xi, yi)->blue + amount;
            int g = original(xi, yi)->green + amount;
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    for (int yi = 0; yi < min(firstImage.height(), secondImage.height()); yi++)
    {
        ...
        {
            int r = (firstImage(xi, yi)->red + secondImage(xi, yi)->red) / 2;
            int b = (firstImage(xi, yi)->blue + secondImage(xi, yi)->blue) / 2;
            int g = (firstImage(xi, yi)->green + secondImage(xi, yi)->green) / 2;

            firstImage(xi, yi)->red = r;
            firstImage(xi, yi)->green = g;
            firstImage(xi, yi)->blue = b;
            secondImage(xi, yi)->red = r;
            secondImage(xi, yi)->blue = b;
            secondImage(xi, yi)->green = g;
        }
    }
    return firstImage;
}
